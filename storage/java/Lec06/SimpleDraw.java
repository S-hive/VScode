...
/** Displays a window and delegates drawing to DrawGraphics. 显示一个窗口,并将绘图任务委托给DrawGraphics。*/
public class SimpleDraw extends JPanel implements Runnable {    
    ...
    /** Paint callback from Swing. Draw graphics using g. 来自Swing的绘制回调。使用g绘制图形*/
    public void paintComponent(Graphics g) {
        super.paintComponent(g);

        // Enable anti-aliasing for better looking graphics启用抗锯齿功能,以获得更美观的图形效果
        Graphics2D g2 = (Graphics2D) g;
        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        draw.draw(g2);
    }

    /** Enables periodic repaint calls.启用定期重绘调用 */

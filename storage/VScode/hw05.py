...
def stair_ways(n):
    ...
    []
    """
    "*** YOUR CODE HERE ***"   #在 Python 中，yield 语句后面不能直接包含一个 for 循环，您不能在列表中使用 for 语法
    if n==0:
        yield []
    if n>0:
...
def is_tree(tree):
    ...
    '''如果给定的树是一棵树，则返回 True,否则返回 False。'''
...
def print_tree(t, indent=0):
    ...
    打印这棵树的一种表示形式，在这种表示形式中，每个节点根据它距离根节点的深度由两个空格乘以该深度进行缩进。

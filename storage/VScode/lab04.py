    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1,amount // price + 1):
                ...
                add(fruits[1:],amount-price*k, cart + display(fruit,k))#display(fruit, k)) 每次遍历都会更新，失去原数据
    add(fruits_to_buy, total_amount, '')        #把原有display函数中结果添加到cart中        
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dist_ax = distance(city, city_a)
    dist_bx = distance(city, city_b)
    if dist_ax == dist_bx:
        return get_name(city_b)
    elif dist_ax < dist_bx:
